...
class oo_config(osv.osv):
    ...
    def __init__(self, pool, cr):
        super(oo_config, self).__init__(pool, cr)
        ########### Run OpenOffice service ###########
        try:
            cr.execute("SELECT host, port FROM oo_config")
            host, port = cr.fetchone()
            oo_service = OpenOffice_service(cr, host, port, allow_raise_errors=False)
            self.set(oo_service)
            logger.log(logging.INFO, "OpenOffice.org connection successfully established")
        except Exception, e:
            cr.rollback()
