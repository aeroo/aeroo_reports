...
class DocumentConverter:
    ...
    def __init__(self, host='localhost', port=DEFAULT_OPENOFFICE_PORT, ooo_restart_cmd=None, allow_raise_errors=True):
        self._host = host
        self._port = port
        self._ooo_restart_cmd = ooo_restart_cmd
        self._allow_raise_errors = allow_raise_errors
        ...
        try:
            ...
            if self._allow_raise_errors:
                raise DocumentConversionException("The url is invalid (%s)" % exception)
            else:
                logger.warning("The url is invalid (%s)" % exception)
            ...
            if self._restart_ooo():
                ...
                try:
                    ...
                    if self._allow_raise_errors:
                        ...
                        logger.warning("Failed to connect to OpenOffice.org on host %s, port %s. %s" % (host, port, exception))
            else:
                if self._allow_raise_errors:
                    raise DocumentConversionException("Failed to connect to OpenOffice.org on host %s, port %s. %s" % (host, port, exception))
                else:
            ...
            if self._allow_raise_errors:
                ...
                logger.warning("Not possible to accept on a local resource (%s)" % exception)
